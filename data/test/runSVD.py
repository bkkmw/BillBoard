...
# top_games_preds = recomm_game_by_surprise(SVD, 'Torsten', unplayed_lst,games, top_n=10)
...
#     print('예측평점:',top_game[1])
#     print()
def recomm_game_perdictions(algo, userID, total_games,games):
    predictions = [algo.predict(str(userID), str(gameId)) for gameId in total_games]
...
def recommend_combination(ids,algo, total_games, games):
    gameset = []
    for id in ids:
        gameset.append(recomm_game_perdictions(algo, id, total_games,games ))

    return gameset
recommend_combination(['GereB', 'Zenfren'],SVD, total_games, games)