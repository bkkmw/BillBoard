...
const ReserveFindAddress = ({
  ...
  useEffect(() => {
    if (myLocal.dong) {
      dispatch(getdongCode(myLocal.dong.dongCode)).then((res) => {
        // console.log(res);
        setCoordinate({
          y: res.payload.coordinate.lat,
          x: res.payload.coordinate.lng,
        });
      });
    }
  }, [myLocal.dong]);
  ...
  const [addressNxt, setAddressNxt] = useState(
    Object.keys(ADDRESS[addressfirst])[0]
  );
            ...
            renderItem={(item) => (
              <List.Item
                ...
                onClick={() => {
                  ...
                  setAddress(
                    `${myLocal.sido.sidoName}/${myLocal.gugun.gugunName}/${item.dongName}`
                  );
                }}
              >
                {item.dongName}
